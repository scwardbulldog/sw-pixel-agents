...
- [ ] `isSymlink()` helper function created (or inline `lstatSync` check)
- [ ] `layoutPersistence.ts` validates symlinks before reading layout file
- [ ] `configPersistence.ts` validates symlinks before reading config file
- [ ] `fileWatcher.ts` validates symlinks before reading JSONL files
- [ ] `assetLoader.ts` validates symlinks in addition to existing path traversal checks
- [ ] Warning logged when symlink is detected and rejected

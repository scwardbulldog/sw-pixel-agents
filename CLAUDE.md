...
src/                          — Extension backend (Node.js, VS Code API)
  ...
  fileWatcher.ts              — fs.watch + polling, readNewLines, /clear detection, terminal adoption, provider-aware session scanning
  transcriptParser.ts         — Claude JSONL parsing: tool_use/tool_result → webview messages
  copilotTranscriptProcessor.ts — Copilot events.jsonl parsing: tool.execution_start/complete → webview messages
...
**Heuristic mode** (fallback): Per-agent 500ms JSONL polling for /clear detection, 1s main scanner for terminal adoption, 3s external scanner (includes Copilot scanner), 30s stale check. Content-based /clear detection (`/clear</command-name>` in first 8KB). Multiple dismissal systems (clearDismissedFiles, dismissedJsonlFiles, seededMtimes, pendingClearFiles).

**JSONL polling** (always active): `readNewLines` routes to provider-specific processor based on `agent.providerId`: Claude uses `processTranscriptLine()`, Copilot uses `processCopilotTranscriptLine()`. Both run in both modes for tool content (status text, animations). Only timer logic (permission 7s, text-idle 5s) is suppressed by `hookDelivered`.

**External session adoption**: `adoptExternalSession()` and `adoptExternalSessionFromHook()` accept `providerId` parameter. This ensures correct transcript parser is used and webview displays proper provider indicator. Global scanner passes `'claude'`, Copilot scanner passes `'copilot'`, hook events pass the provider ID from the hook.

**Extension state per agent**: `id, terminalRef, projectDir, jsonlFile, fileOffset, lineBuffer, activeToolIds, activeToolStatuses, activeSubagentToolNames, isWaiting, providerId`.

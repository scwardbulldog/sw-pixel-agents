/**
 * Formal audit logging module for security-relevant events.
 *
 * Addresses SEC-008: No Formal Audit Logging for Security-Relevant Events
 *
 * Provides structured JSON audit events tagged with [AUDIT] prefix so they
 * can be filtered and forwarded to SIEM / log-aggregation systems.
 * All events are emitted at WARN level so they appear even in production
 * (where INFO-level logging is suppressed).
 *
